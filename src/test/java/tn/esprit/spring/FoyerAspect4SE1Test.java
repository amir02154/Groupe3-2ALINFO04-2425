...
class FoyerAspect4SE1Test {
    ...
    private FoyerAspect4SE1 aspect;
    ...
    @BeforeEach
    void setUp() {
        aspect = new FoyerAspect4SE1();
    }

    @Test
    void testHelloAdvice() throws Exception {
        // Arrange
        ...
        assertDoesNotThrow(() -> aspect.hello(joinPoint));
    }

    @Test
    void testByeAdvice() throws Exception {
        // Arrange
        when(joinPoint.getSignature()).thenReturn(new org.aspectj.lang.Signature() {
            @Override
            public String toShortString() {
        ...
        // Act & Assert - should not throw exception
        assertDoesNotThrow(() -> aspect.bye(joinPoint));
    }

    @Test
    void testExecutionAdvice() throws Throwable {
        // Arrange
        Foyer foyer = new Foyer();
        when(proceedingJoinPoint.proceed()).thenReturn(foyer);

        // Act
        Object result = aspect.execution(proceedingJoinPoint);
    ...
    @Test
    void testExecutionAdviceWithException() throws Throwable {
        // Arrange
        RuntimeException exception = new RuntimeException("Test exception");
        when(proceedingJoinPoint.proceed()).thenThrow(exception);

        // Act & Assert
        assertThrows(RuntimeException.class, () -> aspect.execution(proceedingJoinPoint));
        verify(proceedingJoinPoint).proceed();
    }
}
